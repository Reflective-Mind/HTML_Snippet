...
async function updateView() {
    console.log('Updating view. Token:', !!token, 'Role:', userRole);
    ...
    if (isAdmin) {
        ...
        // Add user toolbar with logout button
        publicView.innerHTML = `
            <div class="bg-light p-2 d-flex justify-content-end" style="position: fixed; top: 0; right: 0; z-index: 1000; background: transparent !important;">
        ...
        // Load the default page for regular users
...
async function loadPublicPage(pageId) {
    try {
        ...
        if (page.navButtons && page.navButtons.length > 0) {
            page.navButtons.forEach(navData => {
                ...
                if (isPreviewMode || !userRole) {
                    // For preview mode and public view
                    button.onclick = (e) => {
                        e.preventDefault();
                    ...
                    button.ondblclick = (e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        showNavButtonEditor(navData.id);
                    };
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="buttonStyle">
                            <option value="btn-outline-primary" ${currentButtonData?.style === 'btn-outline-primary' ? 'selected' : ''}>
                                Outline
                            </option>
                            <option value="btn-primary" ${currentButtonData?.style === 'btn-primary' ? 'selected' : ''}>
                                Solid
                            </option>
                        </select>
                    </div>
...
// Update navigation button click handler to remove simulation
